...
| `n_workers` | no | Worker processes for a trajectory variable's rasterisation (`eddies`), one day per task. Defaults to 4. Conversion runs a month at a time, so the main process holds about a month of daily grids whatever this is; each worker adds only its share of the raw observations plus one day's grids, so lowering it mostly trades speed rather than memory. |
