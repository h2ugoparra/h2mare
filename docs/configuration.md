...
| `derived_vars` | no | Variables computed at convert time and written to the native store, keyed by output name: `{gke: {op: kinetic_energy, source: [ugos, vgos]}}`. `op` is `rolling_std` (one `source`; `window`, an odd number of cells, default 3, gives the square lon/lat box centred on each cell, which uses whatever part of the box holds data) or `kinetic_energy` (`source: [u, v]`, gives `0.5*(u²+v²)`). Sources are named as `source_renames` and the var_key's registered processor leave them (`sst`, not `analysed_sst`). Entries run in order, so one may read an earlier one. Missing sources, a wrong number of them, an even `window` or a misspelt field are refused. By default the result keeps any depth axis its sources have, so a derived 3-D variable needs its own `depth_levels` entry (`ke: [0]` → `ke_0`). To compute and store only some depths, add `depth: [0, 50]`: each level is matched to the nearest source depth (as `depth_levels` does) and written as a 2-D `ke_0`, `ke_50`, so the other levels are never read or stored. Such an entry is listed by those names in `compiled_vars` and must not appear in `depth_levels`; config load refuses that, and refuses two entries or a `depth_levels` column writing the same name. The cost is that changing the levels needs a reconversion, and a per-run `{"ke": [50]}` extraction override does not apply to it (ask for `ke_50` by name). Every output name needs a `variable_attrs` entry like any other. The `sst_std`, `adt_std`, `sla_std` and `gke` layers are declared this way, so removing those entries stops them being written. `bathy` is never converted: its entry (`bathy_std`) is applied by `scripts/bathymetry.py` when it builds the native layers. |
| `boa_fronts` | no | Front-distance layers detected at convert time with the Belkin–O'Reilly algorithm, keyed by output name: `{sst_fdist: {source: sst, threshold: 0.4}}`. Every sea cell gets the great-circle distance (km) to the nearest front pixel found in `source` that day; land is NaN. `source` is named as `source_renames` and the var_key's registered processor leave it (`sst`, not `analysed_sst`) and must be a 2-D field over time/lat/lon — a source with a depth axis is refused, since BOA reads one lat×lon field per day. Keyed by output name, so a store holding several fields can publish a layer for each (`{adt_fdist: {source: adt, …}, sla_fdist: {source: sla, …}}`). `threshold` is BOA's own gradient-magnitude cut, **in the source variable's units** — 0.4 °C for `sst`, 0.06 mg.m-3 for `chl` (Belkin & O'Reilly 2009) — so it does not transfer between variables, and a different detection algorithm would get its own config key rather than a mode flag here. Optional `n_workers` sets the pool size, one day per task (default 10, capped by the host's CPU count and by `H2MARE_MAX_WORKERS`; detection workers are cheap — one lat×lon slab each — which is why the default sits above the eddies one); detection runs a month at a time and stages each month beside `INTERIM_DIR`, so memory follows the month rather than the period. Daily stores only, and config load refuses an hourly one. Every output name needs a `variable_attrs` entry and a place in `compiled_vars` like any other; changing a threshold means re-converting the store, and the layer's `comment` states the value (a test keeps the two in step). The `sst_fdist` and `chl_fdist` layers are declared this way, so removing those entries stops them being written. |
...
| `source_renames` | no | Source variable names renamed to the names this var_key publishes them under: `{analysed_sst: sst}`. This is the map between `source_vars` and `compiled_vars`, and the only place it lives. Applied at convert time **before** the var_key's registered processor, so the processor's own body, `boa_fronts`, `derived_vars` and the CF attrs all name variables the way config does; a variable whose only need is a rename (`mld`) therefore needs no processor at all. Only names change — a variable *computed* from another is `derived_vars`. A rename onto a name the var_key does not publish, onto a name `derived_vars` or `boa_fronts` also writes, two sources onto one name, or a `compiled_vars` still listing a renamed-away source are all refused at config load. The rename *keys* are not checked against `source_vars`, because that field holds what is requested from the provider — for CDS the API's long name (`2m_temperature`) rather than the name the GRIB carries (`t2m`); a key naming a variable the files do not hold fails the convert instead, by name. Changing a target means re-converting the store, and the shipped `sst`, `chl` and `mld` maps are pinned by a test because their processors read the new names. |
...
| `H2MARE_MAX_WORKERS` | no | Ceiling on the **process** pools — BOA front detection and the eddy rasterisation. A *ceiling*, not a value: each site keeps its own measured default (10 and 4) and its own `n_workers` in `config.yaml`, and this only caps them, for a machine smaller than the one those numbers were measured on. Unset (the default) leaves the host's CPU count as the only cap, which applies either way. A value that is not a whole number ≥ 1 is warned about and ignored rather than raised, since `Settings()` runs on any import. It does not touch the thread pools (AVISO FTP downloads, `parquet2csv`, geometry extraction): those wait on the network or the filesystem, so more threads than cores is the point there and a CPU-count cap would only slow them down. |
...
5. To publish a rolling std or a kinetic energy computed from the downloaded fields, declare it under `derived_vars` rather than writing a processor; for a front-distance layer, declare it under `boa_fronts` with the threshold in the source variable's own units.
