...
- `source_renames` (config, applied by `xarray_helpers.rename_source_vars` *before* that processor) — source name → published name (`{analysed_sst: sst}`), the map between `source_vars` and `compiled_vars` and the only place it lives. Everything after it — the processor body, `boa_fronts`, `derived_vars`, `apply_cf_attrs` — names variables the way config does, so a var_key needing nothing but a rename (`mld`) needs no processor at all. Names only; a variable *computed* from another is `derived_vars`. The shipped `sst`/`chl`/`mld` maps are pinned by a test because their processors index the new names.
